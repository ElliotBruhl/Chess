...
public final class Board {
    ...
    public Board(Board b) { //for generating future board states
        this.pastMoves = (HashMap<Long, Integer>) b.getPastMoves().clone(); //deep copy
    ...
    public boolean fastInCheck(int kIndex) { //is current color in check
        if (this.move) {
            return (getAttackMask() & this.bitboards[10]) != 0L;
        }
        return (getAttackMask() & this.bitboards[11]) != 0L;
    }
    public boolean fastInCheck2(int kIndex) {
        return false;
    }
    //game management
    public int gameState() { //0 play on, 1 draw, 2 white win, 3 black win
        ...
        if (!hasLegalMove()) {
            if (fastInCheck(Long.numberOfLeadingZeros(this.move ? this.bitboards[10] : this.bitboards[11]))) {
                return this.move ? 3 : 2;
            }
            return 1;
        }
        //note that all conditions below are ignored for perft
    ...
    public int manageMove(int start, int end) { //HASH FUNCTION DISABLED
        ...
        this.pastMoves.merge(Zobrist.getHash(this), 1, Integer::sum);
        this.move = !this.move;

        return specialMoveFlag;
    }
    public void managePromotion(int file, int bbNum) { //bbNum: pass in -1 to choose piece (GUI), engine passes in 2, 4, 6, 8 for rook, knight, bishop, queen, respectively
        if (bbNum == -1) {
