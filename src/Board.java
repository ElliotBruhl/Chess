...
public final class Board {
    ...
    private final ArrayList<Integer> pastMoves; //for repitition
    ...
    public Board(Board b) { //for generating future board states
        this.pastMoves = (ArrayList<Integer>) b.getPastMoves().clone(); //deep copy
        this.bitboards = b.getBitboards().clone(); //deep copy
        this.move = b.getMove();
    ...
    public int[] getRandomMove() { //random but valid move*
        Random rand = new Random();
        int start = -1;
        int end = -1;
        int[] startIndices = getStartIndices(this.move);
        while (start == -1) {
            int tempIndexS = rand.nextInt(startIndices.length);
            int startIndex = startIndices[tempIndexS];
            long legalMoves = getLegalMoves(new int[]{startIndex})[0];
            if (legalMoves != 0L) {
                start = startIndex;
    ...
    public boolean fastInCheck(int kIndex, long blockers) { //is current color in check
        if (this.move) {
            return (getAttackMask() & this.bitboards[10]) != 0L;
        }
        return (getAttackMask() & this.bitboards[11]) != 0L;
    }
    public long pawnCheck(int kIndex) {
    ...
    public long rookCheck(int kIndex, long blockers) { //traces path of check or 0L if no check
    ...
    public long bishopCheck(int kIndex, long blockers) { //traces path of check or 0L if no check
    ...
    public long getPinnersDiag(int kIndex) {
        long oppBQPos = this.move ? (this.bitboards[7]|this.bitboards[9]) : (this.bitboards[6]|this.bitboards[8]);
        long blockers = getAllofColor(true)|getAllofColor(false);
    ...
    public int getKingIndex() {
        return (Long.numberOfLeadingZeros(this.move ? this.bitboards[10] : this.bitboards[11]));
    }
    public int generatePosHash() { //implement Zobrist hash later
        return 0;
    }
    public int gameState() { //0 play on, 1 draw, 2 white win, 3 black win -- DISABLE SOME DRAW CONDITIONS FOR PERFT
        //stalemate and checkmate
        if (!hasLegalMove()) {
            if (fastInCheck(getKingIndex(), getAllofColor(true)|getAllofColor(false))) {
        ...
        int posCount = 0; //3 fold repitition
        for (int i : this.pastMoves) {
            if (i == pastMoves.get(0)) {
                posCount++;
                if (posCount >= 3) {
                    return 1;
                }
    ...
    public int manageMove(int start, int end) { //HASH FUNCTION DISABLED
        int specialMoveFlag = -1; //0 to 7 - promotion file
        int startBBIndex = getBBfromIndex(start); //type of piece
        //castling
        if (start == 4 && startBBIndex == 10) { // White king
            if (end == 6) {
        ...
        else if (start == 60 && startBBIndex == 11) { // Black king
            if (end == 62) {
                this.bitboards[3] &= ~(1L); // Kingside
                this.bitboards[3] |= (1L << 2);
            }
            else if (end == 58) {
                this.bitboards[3] &= ~(1L << 7); // Queenside
                this.bitboards[3] |= (1L << 4);
            }
        }
        //do en passant capture
        if (end == this.enPassantTarget) {
            if (startBBIndex == 0) { //for white remove black pawn
                this.bitboards[1] &= ~(1L << (63-(end - 8)));
            }
            else if (startBBIndex == 1) { //for black remove white pawn
        ...
        if ((startBBIndex == 0 || startBBIndex == 1) || capturedType != -1) { //irreversible move
            this.halfmoves = 0;
            this.pastMoves.clear();
        }
        if (capturedType != -1) { //capture
            this.bitboards[capturedType] &= ~(1L << (63-end)); //remove captued piece
        }
        else {
            this.halfmoves++;
        }
        //manage promotion
        if (end >= 56 && startBBIndex == 0) { //white
            specialMoveFlag = (end%8); //promotion flag set
            this.bitboards[0] &= ~(1L << (63-start)); //remove pawn
        }
        else if (end <= 7 && startBBIndex == 1) { //black
            specialMoveFlag = (end%8); //promotion flag set
        ...
        //this.pastMoves.add(0, generatePosHash());
        this.move = !this.move;

        return specialMoveFlag;
    }
    public void managePromotion(int file, int bbNum) { //bbNum: pass in -1 to choose piece, engine passes in 2, 4, 6, 8 for rook, knight, bishop, queen, respectively
        if (bbNum == -1) {
            switch (Gui.selectPromotionPiece()) {
                ...
                case "Knight" -> bbNum = 4;
                case "Bishop" -> bbNum = 6;
                default -> throw new Error("Bad Promotion Piece");
            }
        }
    ...
    private void readFEN(String FEN) {
        Map<Character, Integer> FENtoBBindices = Map.ofEntries(
            Map.entry('P', 0),  // White pawns
            Map.entry('p', 1),  // Black pawns
            Map.entry('R', 2),  // White rooks
            Map.entry('r', 3),  // Black rooks
            Map.entry('N', 4),  // White knights
            Map.entry('n', 5),  // Black knights
            Map.entry('B', 6),  // White bishops
            Map.entry('b', 7),  // Black bishops
            Map.entry('Q', 8),  // White queen
            Map.entry('q', 9),  // Black queen
            Map.entry('K', 10), // White king
            Map.entry('k', 11)  // Black king
        );
