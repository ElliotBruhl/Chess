...
public final class Board {
    ...
    public Board(Board b, int start, int end, boolean promoting) throws Exception { //for generating future board states
        this.pastMoves = (HashMap<Long, Integer>) b.getPastMoves().clone(); //shallow copy but wrapper classes immutable so its deep
    ...
    public boolean fastInCheck(int kIndex) { //is current color in check -- faster version later?
        if (this.move) {
    ...
    public boolean manageMove(int start, int end) { //switch statement to BBIndex?
        ...
        this.pastMoves.merge(Zobrist.getHash(this), 1, (_, v) -> v + 1); //adds (key, value) or increment value if present
        this.move = !this.move;
